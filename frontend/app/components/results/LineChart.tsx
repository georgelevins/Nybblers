import type { TimePoint } from "@/app/lib/resultsVisualData";
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${yLabel} by ${xLabel}`}>
      ...
      {markers.map((marker) => {
        ...
        return (
          <g key={`marker-${marker.label}-${marker.index}`}>
            <line
              x1={x}
              y1={padTop}
              x2={x}
              y2={padTop + innerHeight}
              stroke={marker.color ?? "#d04f00"}
              strokeDasharray="6 4"
              strokeWidth="1.2"
            />
            <text x={x + 4} y={padTop + 12} fill={marker.color ?? "#d04f00"} fontSize="11">
              {marker.label}
            </text>
          </g>
        );
