...
import MarketSaturationChart, { type SaturationPoint } from "./MarketSaturationChart";
...
type CardId = "trend" | "subreddits" | "feedback" | "momentum" | "usernames" | "saturation";
...
function normalizeSubredditUsers(source: Record<string, string[]>): SubredditUserGroup[] {
  return Object.entries(source)
    .map(([name, users]) => ({
      name: name.replace(/^r\//i, ""),
      users,
...
export default function ResultsWorkspace({
  ...
  const subredditSource = Object.keys(subreddits).length > 0 ? subreddits : USERS_BY_SUBREDDIT;

  const subredditGroups = useMemo(
    () => normalizeSubredditUsers(subredditSource),
    [subredditSource],
  );

  const subredditStats = useMemo(
    () => normalizeSubredditStats(subredditSource),
    [subredditSource],
  );
  ...
  const totalProfilesFound = subredditGroups.reduce((sum, group) => sum + group.users.length, 0);
  ...
  const saturationPoints: SaturationPoint[] = useMemo(() => {
    const base = trendPoints.slice(-Math.min(24, trendPoints.length));
    const safeLen = Math.max(base.length - 1, 1);
    return base.map((point, index) => {
      const progress = index / safeLen;
      const toolLift = Math.round(progress * 12);
      const projectedTools = Math.round(point.value * 0.58 + toolLift);
      const tools = Math.max(1, Math.min(projectedTools, Math.max(1, point.value - 3)));
      return {
        date: point.date,
        label: point.label,
        needs: point.value,
  ...
  const latestSaturationPoint = saturationPoints[saturationPoints.length - 1] ?? null;
  const latestDemandGap = latestSaturationPoint
    ? latestSaturationPoint.needs - latestSaturationPoint.tools
    : 0;
  const toolCoveragePct = latestSaturationPoint && latestSaturationPoint.needs > 0
    ? (latestSaturationPoint.tools / latestSaturationPoint.needs) * 100
    : 0;
  ...
  function downloadUsersZip() {
    ...
    subredditGroups.forEach((group) => {
      group.users.forEach((username) => {
        rows.push(`${csvEscape(username)},${csvEscape(`r/${group.name}`)}`);
      });
    });
    const usersCsv = rows.join("\n");
    const notes = [
      "Export generated by Remand.",
      `Topic: ${query}`,
      `Profiles exported: ${rows.length - 1}`,
      `Subreddits covered: ${subredditGroups.length}`,
    ].join("\n");

    const zipBlob = createZipBlob([
      { name: "remand_usernames_by_subreddit.csv", content: usersCsv },
      { name: "README.txt", content: notes },
    ]);
    const url = URL.createObjectURL(zipBlob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = "remand-usernames.zip";
    anchor.click();
    ...
    >
      ...
      <section className={styles.resultsMain}>
        ...
        <div className={styles.resultsPanelsViewport}>
          <div className={styles.resultsPanelsTopGrid}>
            <div
          ...
          <div className={styles.resultsPanelsExtra}>
            <article className={`${cardClass("saturation")} ${styles.resultsDataCardWide}`.trim()}>
              <div className={styles.resultsDataCardHead}>
                <h2 className={styles.resultsDataCardTitle}>Market Saturation Curve</h2>
                <button
                  type="button"
                  className={styles.resultsDataCardToggle}
                  onClick={() =>
                    setExpandedCard((current) => (current === "saturation" ? null : "saturation"))
                  }
                  aria-label={expandedCard === "saturation" ? "Collapse market saturation curve" : "Expand market saturation curve"}
                >
                  {expandedCard === "saturation" ? "-" : "+"}
                </button>
              </div>
              <p className={styles.resultsDataCardMeta}>
                Solution density vs demand. If need/problem mentions stay above tool mentions, whitespace exists.
              </p>
              {expandedCard === "saturation" && (
                <p className={styles.resultsDataCardDescription}>
                  Demand gap: <strong>{latestDemandGap}</strong> mentions. Tool coverage:{" "}
                  <strong>{toolCoveragePct.toFixed(1)}%</strong>.
                </p>
              )}

              <div className={styles.saturationLegend}>
                <span className={styles.saturationLegendItem}>
                  <span className={`${styles.saturationLegendDot} ${styles.saturationLegendDotNeed}`} />
                  Need / Problem Mentions
                </span>
                <span className={styles.saturationLegendItem}>
                  <span className={`${styles.saturationLegendDot} ${styles.saturationLegendDotTools}`} />
            ...
            <article className={`${cardClass("usernames")} ${styles.resultsDataCardWide}`.trim()}>
              <div className={styles.resultsDataCardHead}>
                <h2 className={styles.resultsDataCardTitle}>People Talking by Subreddit</h2>
                <button
                  type="button"
                  className={styles.resultsDataCardToggle}
                  onClick={() =>
                    setExpandedCard((current) => (current === "usernames" ? null : "usernames"))
                  }
                  aria-label={expandedCard === "usernames" ? "Collapse usernames list" : "Expand usernames list"}
                >
                  {expandedCard === "usernames" ? "-" : "+"}
                </button>
              </div>
              <p className={styles.resultsDataCardMeta}>
                {totalProfilesFound} profiles across {subredditGroups.length} subreddits.
              </p>
              {expandedCard === "usernames" && (
                <p className={styles.resultsDataCardDescription}>
                  Sorted by subreddit activity. Download the full username list as a zip file.
                </p>
              )}

              <div className={styles.resultsUsersBySubredditList}>
                {subredditGroups.map((group) => (
                  <section key={group.name} className={styles.resultsUsersGroup}>
                    <h3 className={styles.resultsUsersGroupTitle}>
                      r/{group.name} <span>({group.users.length})</span>
                    </h3>
                    <div className={styles.resultsUsersChips}>
                      {group.users.map((username) => (
                        <span key={`${group.name}-${username}`} className={styles.resultsUsersChip}>
                          u/{username}
                        </span>
                      ))}
                    </div>
              ...
              {expandedCard === "usernames" && (
                <button type="button" className={styles.resultsZipButton} onClick={downloadUsersZip}>
                  Download usernames (.zip)
                </button>
              )}
            </article>
          </div>
